...
/**
 * Custom Coil 3 fetcher for:
 * - Audio files → embedded album art
...
class App : Application(), SingletonImageLoader.Factory {
  ...
  override fun onCreate() {
    ...
    applicationScope.launch {
      runCatching {
        ...
        val componentName = ComponentName(
          this@App,
          "app.gyrolet.mpvrx.ui.mediainfo.MediaInfoActivityAlias"
        )
    ...
    applicationScope.launch {
      runCatching { metadataCache.performMaintenance() }
    }
    ...
    applicationScope.launch {
      runCatching { triggerMediaScanOnLaunch() }
    }
  }
  ...
  private fun triggerMediaScanOnLaunch() {
    try {
      if (!shouldRunLaunchMediaScan()) {
        Log.d("App", "Skipped launch media scan; last scan was recent")
        return
      }
      ...
      android.media.MediaScannerConnection.scanFile(
        ...
        Log.d("App", "Launch media scan completed for: $path")
        MediaLibraryEvents.notifyChanged()
      }
      Log.d("App", "Triggered media scan on app launch")
    } catch (error: Exception) {
      Log.e("App", "Failed to trigger media scan on launch", error)
    }
  }
  ...
  private fun shouldRunLaunchMediaScan(): Boolean {
    ...
    if (now - lastScan < LAUNCH_SCAN_INTERVAL_MS) return false
